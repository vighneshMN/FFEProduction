...
var app=angular.module('Facilitator', ['ngRoute','ngAnimate','ngMaterial','number','text','textarea']);
...
app.controller('FacilitatorProfile', function($scope, $timeout, $window, $location,$element) {
 ...
 $scope.selectedTab=0;
    $scope.selectedTabIndex=function(index){
    ...
    $scope.tabs =function(){
        ...
        if($scope.selectedTab == 1){
             ...
             $scope.switchInternal(0,0);$scope.selectedTabIndex(1);$scope.showTabcontent(1);$scope.getAttStudents();
             document.getElementById("listings").style.display="block";   
             document.getElementById("Basic").style.display="none";
         }else if($scope.selectedTab == 2){   
             $scope.switchInternal(1,1);$scope.selectedTabIndex(2);$scope.showTabcontent(1);$scope.getAttStudents();
                ...
                AP_Application_CTRL.viewStudentProfile($scope.contantId,function(result, event){
                    if(event.status){ 
                        ...
                        if($scope.studentDetails.Applications__r != undefined){
                                    ...
                                    if($scope.studentDetails.Applications__r.records[i].RecordType.DeveloperName =='Scholarship'){
                                       ...
                                       // $scope.application__c.Id 										= $scope.studentDetails.Applications__r.records[i].Id;
                                        if($scope.studentDetails.Applications__r.records[$scope.activeIndex].FacilitatorInputDone__c == true || $scope.studentDetails.Applications__r.records[$scope.activeIndex].Internal_Status__c=='Pending FFE Staff Review' || $scope.studentDetails.Applications__r.records[$scope.activeIndex].Internal_Status__c=='Rejected by Facilitator'){
                                            ...
                                            $scope.application__c.Scholarship_other_than_FFE__c 	        = $scope.studentDetails.Applications__r.records[i].Scholarship_other_than_FFE__c.toString();
                                        }
                            ...
                            $scope.application__c.Id  = $scope.studentDetails.Applications__r.records[0].Id;

                            if($scope.studentDetails.Applications__r.records[0].RecordType.DeveloperName =='Scholarship'){
                                $scope.isPrelim = true;
                            }else{
                                $scope.isPrelim = false;
                            }
